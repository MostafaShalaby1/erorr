...
async def hfmm(_, message):
    ...
    if status in ["OFF", "Off", "off"]:
        ...
        if message.chat.id in DISABLED_GROUPS:
            await lel.edit("وضع الصيانة نشط بالفعل في هذه الدردشة")
            return
        DISABLED_GROUPS.append(message.chat.id)
        await lel.edit(
            f"تم تمكين وضع الصيانة الآن في **{message.chat.title}** Chat"
        )
    else:
        await message.reply_text(
            "أنا فقط أدرك `/Maintainmode on` and /Maintainmode `off only"
        )
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"<b>🛑 خطأ انتظار الفيضان 🛑</b> \n\Hey {user.first_name}, لم يتمكن برنامج حساب المساعد من الانضمام إلى مجموعتك بسبب كثرة طلبات الانضمام. تأكد من عدم حظر حساب المساعد في المجموعة وحاول مرة أخرى لاحقًا!"
                    )
    try:
        ...
        await lel.edit(
            f"<i>Hey {user.first_name}, حساب المساعد ليس في هذه المجموعه, اطلب من المشرف الإرسال /play الأمر لأول مرة لإضافته.</i>"
        )
        return
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton("🚨 قناة البوت", url="t.me/vod_ika0"),
                    InlineKeyboardButton("📡 مبرمج السورس", url="t.me/v_o_d_i_k_a"),
                ],
